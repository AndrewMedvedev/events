...
from .db.models import EventModel, NewsModel, VisitorModel
...
class EventListResponse(BaseModel):
    ...
    def to_dict(self) -> dict:
        return {
            "event": [e.to_dict() for e in self.events]
        }
...
class UserSchema(BaseModel):
    ...
    def from_model(cls, user: VisitorModel) -> UserSchema:
        return cls(
            ...
            unique_string=user.unique_string
        )
...
class UserEventSchema(BaseModel):
    ...
    def create(cls, event: list) -> UserEventSchema:
        return cls(
            user_event=[u.from_model(u) for u in event]
        )

    def to_dict(self) -> dict:
        return {
            "user_event": [u.to_dict() for u in self.user_event]
        }
