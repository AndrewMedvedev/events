...
async def get(user_id: int) -> Response:
    """Изменился возвращаемый ответ, раньше был список, сейчас словарь"""
...
async def verify_visitor(request: Request, unique_string: str) -> HTMLResponse:
    ...
    if result is not None:
        return templates.TemplateResponse("register", {"request": request, **result})

    return templates.TemplateResponse("not_register", {"request": request})
